...
import { Typography, Paper, Box, TextField, Button, Stack, Alert, CircularProgress, Divider, LinearProgress, Tabs, Tab, useTheme } from '@mui/material';
...
export default function DocumentSummary() {
  ...
  const isDark = theme.palette.mode === 'dark'; // Temanın karanlık modda olup olmadığını kontrol ediyoruz
  ...
  return (
    <Box sx={{ maxWidth: 850, mx: 'auto', p: { xs: 1, md: 3 } }}>
      ...
      <Paper elevation={3} sx={{ borderRadius: 3, overflow: 'hidden' }}>
        ...
        <Box sx={{ borderBottom: 1, borderColor: 'divider', bgcolor: isDark ? 'background.default' : '#f8fafc' }}>
          <Tabs 
            value={activeTab} 
            ...
            sx={{
              ...
              '& .Mui-selected': { 
                fontWeight: 'bold', 
                bgcolor: isDark ? 'rgba(144, 202, 249, 0.08)' : 'primary.50' // Dark mod için hafif transparan mavi
              }
            }}
          >
              ...
              <Paper 
                ...
                sx={{ 
                  ...
                  bgcolor: selectedFile 
                    ? (isDark ? 'rgba(102, 187, 106, 0.08)' : 'success.50') // Dark modda göz yormayan yeşil ton
                    : 'background.default',
                  textAlign: 'center',
                  cursor: 'pointer',
                  transition: '0.2s',
                  '&:hover': { borderColor: 'primary.main', bgcolor: isDark ? 'action.hover' : 'action.hover' }
                }}
                component="label"
              >
          ...
          {summary && (
            <Box sx={{ mt: 5, animation: 'fadeIn 0.6s ease-out' }}>
              <Paper 
                ...
                sx={{ 
                  p: 4, 
                  bgcolor: isDark ? 'background.paper' : '#f8fbff', // Dark mod düzeltmesi
                  borderTop: '5px solid', 
                  borderColor: 'primary.main', 
                  borderRadius: 2 
                }}
              >
                <Typography variant="h5" fontWeight="bold" color={isDark ? 'primary.light' : 'primary.dark'} display="flex" alignItems="center" gap={1.5} mb={2}>
                ...
                <Typography 
                  ...
                  sx={{ 
                    whiteSpace: 'pre-wrap', 
                    lineHeight: 1.9, 
                    color: 'text.primary', // Artık koyu modda bembeyaz, aydınlıkta simsiyah olacak
                    fontSize: '1.1rem' 
                  }}
                >
                  {summary}
